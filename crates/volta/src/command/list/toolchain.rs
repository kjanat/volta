...
impl Lookup {
    fn version_from_spec(&self) -> impl Fn(&PlatformSpec) -> Option<Version> + '_ {
        move |spec| match self {
            Self::Runtime => Some(spec.node.clone()),
            Self::Npm => spec.npm.clone(),
            Self::Pnpm => spec.pnpm.clone(),
            Self::Yarn => spec.yarn.clone(),
        }
    }
