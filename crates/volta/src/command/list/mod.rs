...
impl Source {
    fn allowed_with(&self, filter: &Filter) -> bool {
        match filter {
            Filter::Default => self == &Self::Default,
            Filter::Current => matches!(self, Self::Default | Self::Project(_)),
            Filter::None => true,
        }
    }
}
...
impl fmt::Display for Source {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            "{}",
            match self {
                Self::Project(path) => format!(" (current @ {})", path.display()),
...
impl Package {
    fn new(config: &PackageConfig, source: &Source) -> Self {
        ...
        match source {
            ...
            Source::Project(path) => Self::Project {
                name: details.name,
                tools: config.bins.clone(),
                path: path.clone(),
            },
            Source::None => Self::Fetched(details),
        }
    }

    fn from_inventory_and_project(project: Option<&Project>) -> Fallible<Vec<Self>> {
...
impl FromStr for Subcommand {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(match s {
            ...
            "pnpm" => Self::Pnpm,
            "yarn" => Self::Yarn,
            s => Self::PackageOrTool { name: s.into() },
        })
    }
}
