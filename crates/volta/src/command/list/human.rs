...
static TEXT_WIDTH: std::sync::LazyLock<usize> =
    std::sync::LazyLock::new(|| text_width().unwrap_or(MAX_WIDTH));
...
pub(super) fn format(toolchain: &Toolchain) -> Option<String> {
    ...
    Some(match toolchain {
        ...
        Toolchain::Active {
            ...
            package_managers,
            packages,
        } => display_active(runtime.as_deref(), package_managers, packages),
        Toolchain::All {
            runtimes,
            package_managers,
            packages,
...
fn display_tool(tool: &str, host_packages: &[Package]) -> String {
    if host_packages.is_empty() {
        format!(
            "⚡️ No tools or packages named `{tool}` installed.
...
See `volta help install` for details and more options."
        ...
        format!("⚡️ Tool `{tool}` available from:\n\n{versions}")
    }
}
...
fn format_package(package: &Package) -> String {
    match package {
        ...
        Package::Fetched(details) => {
            ...
            let footer_message =
                format!("To make it available to execute, run `volta install {package_info}`.");
            format!("{package_info}\n\n{footer_message}")
        }
    }
}
...
mod tests {
    ...
    mod active {
        ...
        #[test]
        fn no_runtimes() {
            let runtime: Option<Box<Node>> = None;
