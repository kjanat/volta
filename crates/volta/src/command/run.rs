...
use log::warn;
use volta_core::error::{ExitCode, Fallible, report_error};
use volta_core::platform::{InheritOption, Overrides};
...
impl Run {
    ...
    fn parse_platform(&self, session: &mut Session) -> Fallible<Overrides> {
        let node = self
            .node
            .as_ref()
        ...
        let npm = match (self.bundled_npm, &self.npm) {
            ...
            (false, Some(version)) => npm::resolve(version.parse()?, session)?
                .map_or(InheritOption::Inherit, InheritOption::Some),
        };
    ...
    fn parse_envs(&self) -> HashMap<&str, &str> {
        self.envs.iter().filter_map(|entry| {
            ...
            match (key_value.next(), key_value.next()) {
                (Some(key), Some(value)) if !key.eq_ignore_ascii_case("PATH") => Some((key, value)),
                (Some(key), Some(_)) => {
                    warn!("Ignoring {key} environment variable as it will be overwritten when executing the command");
                    None
                }
                _ => None,
            }
