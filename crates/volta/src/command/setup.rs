...
mod os {
    ...
    pub fn setup_environment() -> Fallible<()> {
        ...
        let home_in_path = env::var_os("PATH")
            .and_then(|paths| env::split_paths(&paths).find(|p| p == home.shim_dir()));
...
If you run into problems running Volta, create {suggested_bash_profile} and run `volta setup` again."
            );
        }
    }
    ...
    fn write_profile_sh(path: &Path, contents: &str, volta_home: &str) -> io::Result<()> {
    ...
    fn write_profile_fish(path: &Path, contents: &str, volta_home: &str) -> io::Result<()> {
        let mut file = File::create(path)?;
        write!(
            file,
