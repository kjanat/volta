...
download_release_from_repo() {
  ...
  local archive_url="${_release_url}/download/v${version}/${filename}"
...
update_profile() {
  ...
  local uname_str
  uname_str="$(uname -s)"
  detected_profile="$(detect_profile "$(basename "/${SHELL}")" "${uname_str}" )"
  local path_str
  path_str="$(build_path_str "${detected_profile}" "${profile_install_dir}")"
  ...
  if [[ -z "${detected_profile-}" ]] ; then
    ...
    if ! command grep -qc 'VOLTA_HOME' "${detected_profile}"; then
      command printf "%s" "${path_str}" >> "${detected_profile}"
    else
...
arch="$(uname -m)"
check_architecture "${version_to_install}" "${arch}" || exit 1
